...
for (i in 1:length(mediators)){
  ...
  if (mediator != "log_REI"){
    ...
    model.em <- svyglm(as.formula(paste(mediator, "~ Age + Gender + BMI +",
  ...
  model.emy <- svyglm(as.formula(paste("inc_diab", "~ Age + ", mediator, " + Gender + BMI +",
                                       "PC_1 + PC_2 + PC_3 + PC_4 + PC_5 +",
                                       cur_prs)),
...
write.csv(res, file = paste0("/Results/DM_PRS_OSA_inc_DM_mediation_inds_visit_2_for_", PRS, "_quantiles_adj_for_BMI.csv"), row.names = FALSE)
